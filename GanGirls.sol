...
contract GanGirls is ERC721URIStorage, Ownable {
    using Counters for Counters.Counter;

    Counters.Counter private _tokenIdCounter;

    mapping(address => bool) loves;

    constructor() ERC721("GanGirls", "GANG") {}
    ...
    function _baseURI() internal pure override returns (string memory) {
        return "ipfs://Qmc27vCEHvTxbSqAkbh53Q7YL6MSzyRUu5LbmNcRwerNds/";
    }

    function mint() public payable {
        require(msg.value >= 0.069 ether, "fee is not correct.");
        require(_tokenIdCounter.current() < 106, "all the GANs are minted.");

        if(loves[msg.sender]){
            //refund
            (bool sent, bytes memory data) = payable(msg.sender).call{value: msg.value}("");
            require(sent, "Failed to send Ether");
            loves[msg.sender] = false;
        }

        _tokenIdCounter.increment();
        uint256 tokenId = _tokenIdCounter.current();

        string memory tokenURI = string(abi.encodePacked( toString(tokenId),  ".json"));
    ...
    function ownerLovesYou(address[] calldata _loves) public onlyOwner{
        for(uint x; x < _loves.length; x++){
            loves[_loves[x]] = true;
        }
    }
